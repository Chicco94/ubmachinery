...
class Snap7_Machine(Costumer_Machine):
    ...
    def read_memory(self, plc, area, db_number, byte, bit, datatype, length=None):
        ...
        try:
            if datatype == "string":
                return plc.read_area(areas[area], db_number, byte, length).decode(
                    "utf-8"
                )
    ...
    def write_memory(
        self, plc, area, db_number, byte, bit, datatype, value, length=None
    ):
        """Legge un area di memoria:
                - plc: client già connesso
                - db_number: numero db
    ...
    def connect(self):
        ...
        try:
            ...
            self.client.connect(address=address, rack=rack, slot=slot, tcpport=tcpport)
