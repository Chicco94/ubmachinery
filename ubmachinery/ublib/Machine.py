...
class Machine(Thread):
        ...
        while not stop:
            ...
            if commessa_attuale and commessa_attuale == commessa_attiva:
                ...
                if self.close_commessa_condition(commessa_attiva, machine_data):
                    ...
                    self.set_db_data(
                        self.update_internal_data_before_save_to_db(
                            commessa_attiva, machine_data
                        )
                    )
                    commessa_attiva = CommesseMacchinario()
    ...
    # ---------------------------------------------------------------------------------------------------------
    def close_commessa_condition(
        self, commessa_attiva: CommesseMacchinario, machine_data: AnalisiMacchinario
    ):
    ...
    def generate_internal_data(
        self, commessa_attiva: CommesseMacchinario = None
    ) -> CommesseMacchinario:
    ...
    def update_internal_data_before_save_to_db(
        ...
        return_data.an_tempoEffettivo = (
            data_fine - commessa_attiva.co_dataInizio
        ).seconds * 1000  # tempo in millisecondi
        return return_data
